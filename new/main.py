...
def update_chart():
    ...
    try:
        ...
        if df.empty:
            print("Received empty DataFrame, skipping chart update.")
            chart.spinner(False)  # Still need to turn off spinner
            return  # Don't try to update with no data

        # 1. Delete old indicator lines FIRST
        if current_lines:
            print(f"Deleting {len(current_lines)} old indicator lines.")
            for l in current_lines:
                try:
                    l.delete()
                except Exception as e:
                    print(f"Warning: Error deleting line object: {e}")
            current_lines = []  # Clear the list after attempting deletion

        # 2. Set the main candlestick data
        print("Setting new main chart data.")
        chart.set(df)
        ...
        # 4. Calculate and add new indicator lines
        print("Calculating and adding indicator lines.")
        # SMA 50
        ...
        if not sma_data.empty:
            ...
            current_lines.append(sma_line)

        # EMA 10
        ema_10_data = pd.DataFrame(
            {
                "time": df["date"],
                "EMA 10": df["close"].ewm(span=10, adjust=False).mean(),
            }
        ).dropna()
        if not ema_10_data.empty:
            ema_10_line = chart.create_line(name="EMA 10", color="#FF69B4")
            ema_10_line.set(ema_10_data)
            current_lines.append(ema_10_line)

        # EMA 20
        ema_20_data = pd.DataFrame(
            {
                "time": df["date"],
                "EMA 20": df["close"].ewm(span=20, adjust=False).mean(),
            }
        ).dropna()
        if not ema_20_data.empty:
            ema_20_line = chart.create_line(name="EMA 20", color="#1E90FF")
