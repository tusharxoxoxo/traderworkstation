...
const char* EDecoder::processExecutionDetailsMsgProtoBuf(const char* ptr, const char* endPtr) {
	...
	Contract contract;
	if (executionDetailsProto.has_contract()) {
		contract = EDecoderUtils::decodeContract(executionDetailsProto.contract());
	}

	// extract execution fields
	Execution execution;
	if (executionDetailsProto.has_execution()) {
		execution = EDecoderUtils::decodeExecution(executionDetailsProto.execution());
	}
